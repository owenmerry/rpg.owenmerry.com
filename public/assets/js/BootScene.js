class BootScene extends Phaser.Scene {
  ...
  setupMusicControls() {
      ...
      document.getElementById('play').addEventListener('click', function () {
        ...
        player.playVideo();
        this.socket.emit('musicUpdate',{action:'play',seekTo:player.getCurrentTime()});
      }.bind(this));
      ...
      document.getElementById('add-song-button').addEventListener('click', function () {
        ...
        this.socket.emit('musicUpdate',{action:'changeVideo',videoID:newVideoInput.value});
        newVideoInput.value = '';
      }.bind(this));
      document.getElementById('volume-button').addEventListener('click', function () {
        const volumeInput = document.getElementById('volume-input');
        player.setVolume(volumeInput.value);
        this.socket.emit('musicUpdate',{action:'volume',volume:volumeInput.value});
      }.bind(this));
  }
