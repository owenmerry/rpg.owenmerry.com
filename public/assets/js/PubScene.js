class PubScene extends Phaser.Scene {
  init(data) {
  ...
  create() {
    ...
    navigator.mediaDevices
      ...
      .catch((err) => {
        console.log("video had an error", err);
      });
  ...
  createMap() {
    ...
    var tiles = this.map.addTilesetImage(
      "pixel_house_set_tiles",
      "house-tiles",
      16,
      16,
      0,
    ...
    var items = this.map.addTilesetImage(
      "pixel_house_set_1.3.4",
      "house-items",
      16,
      16,
      0,
    ...
    // creating the layers
    this.layerBackgroundFloor = this.map.createStaticLayer(
      "background-floor",
      tiles,
      0,
      0
    );
    this.layerBackgroundRoom = this.map.createStaticLayer(
      "background-room",
      tiles,
      0,
      0
    );
    this.layerBackgroundTop = this.map.createStaticLayer(
      "background-top",
      items,
      0,
      0
      ...
      if (this.keyEnter.isDown && 1 === 0) {
        ...
        this.otherPlayers.getChildren().forEach(
          function (otherPlayer) {
            if (
              this.physics.overlap(this.container, otherPlayer) &&
              foundChat === false
            ) {
              this.socket.removeAllListeners();
              this.socket.emit("createOrJoinRoom", {
                addUser: otherPlayer.playerId,
              });
              this.scene.start("ChatScene", {
                socket: this.socket,
