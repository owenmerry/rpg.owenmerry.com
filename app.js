...
io.on("connection", function (socket) {
  ...
  const shuffled = startPosition.sort(() => 0.5 - Math.random());
  ...
  socket.on("playerMovement", function (movementData) {
    if (users[socket.id].room === "" || 1 === 1) {
      ...
      if (rooms[users[socket.id].room].players[socket.id]) {
        ...
        rooms[users[socket.id].room].players[socket.id].flipX =
          movementData.flipX;

        socket.broadcast.emit("playerMoved", {
          ...rooms[users[socket.id].room].players[socket.id],
          from: "room",
        });
      }
    }
  ...
  socket.on("sendAnswer", function (answerData) {
    ...
    if (
      conversation[answerData.to] &&
      conversation[answerData.to].users[answerData.from]
    ) {
      conversation[answerData.to].users[answerData.from].answer =
        answerData.answer;
      // emit to user for answer
      io.to(answerData.to).emit(
        "reciveAnswer",
        conversation[answerData.to].users[answerData.from]
      );
    }
  ...
  socket.on("leaveRoom", function () {
    ...
    if (rooms[removeRoom]) {
      ...
      if (removeIndex > -1) {
        rooms[removeRoom].users.splice(removeIndex, 1);
      }
    }
  ...
  socket.on("userReady", function (readyData) {
    io.to(readyData.to).emit("chatReady", {
      from: socket.id,
      to: readyData.to,
    });
  });
...
app.get("/", function (req, res) {
  res.sendFile(__dirname + "/public/pub.html");
});
